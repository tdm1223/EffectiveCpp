...
## 객체의 모든 부분을 빠짐없이 복사하기
- 기존에 클래스를 설계할때 복사 함수를 직접 구현한다면 나중에 변수가 추가될때 부분 복사가 일어날 수 있다.
- 클래스에 멤버를 추가한다면, 추가한 데이터 멤버를 처리하도록 복사 함수를 다시 작성할 수 밖에 없다.
- 이런 상황에서 일언반구라도 해주는 컴파일러는 거의 없다(당연한거인듯??)
