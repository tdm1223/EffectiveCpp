...
2. Person 객체의 포인터를 담을 공간만 할당하면 된다.
    - 정의부에 대한 의존성을 선언부에 대한 의존성으로 바꾸는 것이다.

- 객체 참조자 및 포인터로 충분한 경우네느 객체를 직접 쓰지 않는다.
- 클래스 정의 대신 클래스 선언에 최대한 의존하도록 만든다.
- 선언부와 정의부에 대해 별도의 헤더 파일을 제공한다.

- 핸들 클래스와 인터페이스 클래스를 통해 구현부로부터 인터페이스를 뗴어 놓을수 있고 이를 통해 파일들 사이의 컴파일 의존성을 완화시킬 수 있다.

### 핸들 클래스
- 핸들 클래스의 멤버 ㅎ함수를 호출하면 구현부 객체의 데이터까지 가기 위해 포인터를 타야 한다.
...
### 인터페이스 클래스
- 호출되는 함수가 전부 가상함수이다.
    - 함수 호출이 일어날 때마다 가상 테이블 점프에 따라는 비용이 소모된다.
- 인터페이스 클래스에서 파생된 객체는 가상 테이블 포인터를 지니고 있어야 한다.
- 인라인 함수의 도움을 제대로 끌어내기 힘들다.
