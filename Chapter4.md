...
- 표준에서 기본적으로 제공하는 `swap`은 복사만 제대로 지원하는 타입이기만 하면 어떤 타입의 객체이든 맞바꾸기 동작을 수행해 준다.
- 한 번 호출에 복사가 **세 번** 일어난다.
- 세 번 복사로 인해 느리게 동작할 여지가 있다면 public `swap 멤버함수`를 제공해야 한다.
    - 이 함수는 절대로 예외를 던져서는 안된다.
        - swap을 진짜 쓸모 있게 응용하는 방법들 중에 클래스가 강력한 예외 안전성 보장을 제공하도록 도움을 주는 방벙이 있기 때문이다.
- `swap 멤버함수`을 제공했으면, 멤버를 호출하는 비멤버 `swap`도 제공해야 한다.
- 새로운 클래스를 만들면 해당 클래스에 대해서 `std::swap`특수화 버전을 준비한다.
- 사용자 입장에서 `swap`을 호출할 때는, `std::swap`에 대한 using 선언을 넣어 준후에 네임스페이스 한정 없이 `swap`을 호출한다.
```cpp
template<typename T>
